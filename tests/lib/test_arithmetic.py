...
def test_Sum_01(test_name, debug_graph, dtype):
...
@mark.parametrize("dtype", ("d", "f"))
def test_Difference_01(test_name, debug_graph, dtype):
    arrays_in = tuple(arange(12, dtype=dtype) * i for i in (1, 2, 3))
...
@mark.parametrize("dtype", ("d", "f"))
def test_Product_01(test_name, debug_graph, dtype):
    arrays_in = tuple(arange(12, dtype=dtype) * i for i in (1, 2, 3))
...
def test_ProductShifted_01(test_name, debug_graph, dtype: str, scaled: bool):
    arrays_in = tuple(arange(12, dtype=dtype) * i for i in (1, 2, 3))
    shift = 1.23245
...
@mark.parametrize("dtype", ("d", "f"))
def test_Division_01(test_name, debug_graph, dtype):
...
def test_Powers_01(test_name, debug_graph, function, dtype):
    if function == square:
...
def test_Abs(test_name, debug_graph, data, dtype):
